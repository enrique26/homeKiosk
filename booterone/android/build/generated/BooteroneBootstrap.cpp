...
#define TAG "com.ibilis.booterone"

using namespace v8;

static Persistent<Object> bindingCache;

static void Booterone_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Booterone.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::BooteroneBindings::lookupGeneratedInit(
...
static void Booterone_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Booterone_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Booterone_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::BooteroneBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry BooteroneBinding = {
	"com.ibilis.booterone",
	Booterone_init,
	Booterone_dispose
};
...
Java_com_ibilis_booterone_BooteroneBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.ibilis.booterone", &BooteroneBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::BooteroneBindings::lookupGeneratedInit));
}
